  ...
  <xsl:template name="token">
    ...
    <script type="text/babel">
      ...
      formatTime(d) {
        //let date = d
        let date = new Date()
        //hhh we should move this hack into native code. Use a better API and convert the timezone? This is mathematicall -8hrs to the time (if in singapore), so the Javascript should be passed the UTC time? So strange
